...
function edit(key, value) { if(match?.active)return notify('Use the guided turn actions, or switch to Build to edit the design.'); const o = current(); if(!o) return; const copy = structuredClone(game); copy.objects.find(x => x.id === o.id)[key] = value; if(copy.runtime?.kind==='routing'&&copy.runtime.spaceIds.includes(o.id)&&key==='rotation'){if(value%90)return notify('Linked gutter tiles rotate in 90° steps.');const i=copy.runtime.spaceIds.indexOf(o.id);copy.runtime.directions[i]=(copy.runtime.directions[i]-value/90+8)%4;} try { validateGame(copy); } catch(e) { notify(e.message); inspector(); return; } checkpoint(); game=validateGame(copy); refresh(); }
...
function rebuild() {
  ...
  targets.length = 0;
  const doc=previewGame||match?.projection()||game;
  for(const o of doc.objects) {
    if(o.type === 'card' && o.deckId && !o.drawn) continue;
    const g = new THREE.Group(); g.position.set(o.x,baseY(o,doc),o.z); g.rotation.y = THREE.MathUtils.degToRad(o.rotation);g.userData.componentId=o.id; world.add(g);
    const parts=o.parts||[];
    if(parts.length){
      for(const p of parts){
        ...
        body.position.set(p.x,p.y,p.z);body.rotation.set(...[p.rx,p.ry,p.rz].map(THREE.MathUtils.degToRad));body.castShadow=true;body.receiveShadow=true;body.userData.id=o.id;g.add(body);targets.push(body);
        if(o.id===(match?.active?match.selectedId:selected)){const lines=new THREE.LineSegments(new THREE.EdgesGeometry(body.geometry),new THREE.LineBasicMaterial({color:'#e99a46'}));body.add(lines);}
      }
    }else{
      const material=new THREE.MeshStandardMaterial({color:o.color,roughness:.72,metalness:.02});
      const body=new THREE.Mesh(geometry(o),material);body.position.y=o.height/2;body.castShadow=true;body.receiveShadow=true;body.userData.id=o.id;g.add(body);targets.push(body);
  ...
  if(match?.active&&match.pathIds.length){
    ...
    if(points.length>1){const path=new THREE.Line(new THREE.BufferGeometry().setFromPoints(points),new THREE.LineBasicMaterial({color:'#318dc5',depthTest:false}));path.renderOrder=5;world.add(path);}
    if(match.state.turn!==lastAnimatedTurn){lastAnimatedTurn=match.state.turn;waterMotion={start:performance.now(),points,group:world.children.find(o=>o.userData.componentId===doc.runtime.waterId)};}else waterMotion=null;
    points.forEach((p,i)=>{const dot=new THREE.Mesh(new THREE.SphereGeometry(.10,12,8),new THREE.MeshBasicMaterial({color:'#319bce'}));dot.position.copy(p);world.add(dot);});
  }
}
function init3D() {
  try {
    ...
    renderer.domElement.addEventListener('pointerup',e=>{
      ...
      if(match?.active){if(hit)match.selectObject(hit.object.userData.id);return;}
    ...
    renderer.setAnimationLoop(()=>{if(mode!=='print'){controls.update();if(waterMotion?.group&&!matchMedia('(prefers-reduced-motion: reduce)').matches){const m=waterMotion,t=Math.min(1,(performance.now()-m.start)/1500),index=t*Math.max(0,m.points.length-1),a=m.points[Math.floor(index)],b=m.points[Math.min(m.points.length-1,Math.floor(index)+1)];if(a&&b){m.group.position.x=a.x+(b.x-a.x)*(index%1)+.7;m.group.position.z=a.z+(b.z-a.z)*(index%1)+.7;}if(t===1)waterMotion=null;}sizeLabels();renderer.render(scene,camera);}});
...
function inspector() {
  ...
  $('delete').onclick=()=>{if(game.runtime)return notify('This game has linked rules. Ask the assistant to remove the component and update its rules together.');checkpoint();removeObject(game,o.id);selected=null;refresh();};
...
function applyLayout(layout) {
  if(game.runtime)return notify('This board has linked rules. Ask the assistant to change its layout and rules together.');
...
function printView() {
  ...
  if(game.runtime){$('print-pages').innerHTML=guidedPrintKit(game);return;}
...
$('undo').onclick=()=>{if(match?.active||!history.length)return;const previous=JSON.parse(history.pop());game=previous.game;selected=previous.selected;mark();refresh();};
$('new-game').onclick=()=>{if((dirty||game.objects.length)&&!confirm('Start a new game and design conversation? Save first to keep your work.'))return;chat?.reset();match?.reset();checkpoint();game=blankGame();selected=null;setMode('build');fit();};
...
$('edit-rules').onclick=()=>{$('rule-fields').innerHTML=Object.entries(ruleLabels).map(([key,label])=>`<label for="rule-${key}">${label}</label><textarea id="rule-${key}" maxlength="8000" placeholder="${key==='victory'?'How does your game end?':'Write your '+label.toLowerCase()+'…'}">${esc(game.rules[key])}</textarea>`).join('');$('rule-fields').querySelectorAll('textarea').forEach(el=>el.readOnly=Boolean(game.runtime));$('save-rules').hidden=Boolean(game.runtime);$('rules-dialog').querySelector('p.muted').textContent=game.runtime?'These instructions come from the same rules configuration that runs guided play. Change the game through the assistant to revise its mechanics.':'Use your own mechanics. Rules travel with your saved game.';$('rules-dialog').showModal();};
...
assess.onclick=()=>{
 const report=evaluateGame(previewGame||game);
 review.innerHTML=`<form method="dialog"><button aria-label="Close playtest report">Close</button></form><div class="eyebrow">BOUNDED RULES CHECK</div><h2>Playtest diagnostics</h2>${report.supported?`<p>${report.matches} matches completed. Seat wins: ${report.wins.join(' / ')}. Ties: ${report.ties}. Average turns: ${report.meanTurns}.</p><ul>${report.findings.map(f=>`<li>${esc(f)}</li>`).join('')}</ul><p>${esc(report.limitations)}</p><button class="primary" id="improve-tested-game">Ask Astra to improve this draft</button><p>One review at a time. Inspect and apply the proposal, then run this check again. No background model calls.</p>`:`<p>${esc(report.summary)}</p>`}`;
 review.querySelector('#improve-tested-game')?.addEventListener('click',()=>{review.close();setMode('build');chat.submit('Review this bounded diagnostic report and the attached board. Propose one targeted improvement if evidence warrants it; otherwise recommend a specific human playtest. Preserve unrelated components. Explain your hypothesis and what to measure next. Report: '+JSON.stringify({...report,games:undefined}));});review.showModal();
};
match=installMatch({getGame:()=>game,onChange:()=>{rebuild();if(match?.state?.finished)fit();},onReview:report=>{setMode('build');chat.submit('Review this actual match record and the current design. Identify one specific hypothesis to test or a targeted improvement. Distinguish observed behavior from guesses about fun or balance. Preserve unrelated parts. Match: '+JSON.stringify(report));}});
init3D();refresh();
chat=installDesignChat({
 ...
 onApply:draft=>{const valid=validateGame(draft);checkpoint();game=valid;match?.reset();previewGame=null;selected=null;document.body.classList.remove('previewing');setMode('build');fit();notify('AI proposal applied. Undo is available.');}
});
