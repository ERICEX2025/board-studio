...
function createLabel(text, width = 3) {
  const c = document.createElement('canvas'); c.width = 512; c.height = 128;
  const ctx = c.getContext('2d'); ctx.font = '500 29px sans-serif'; ctx.textAlign = 'center'; ctx.fillStyle = '#dddddd'; ctx.shadowColor = '#181818'; ctx.shadowBlur = 8;
  ctx.fillText(text.length > 26 ? text.slice(0,25)+'…' : text,256,60);
  const texture = new THREE.CanvasTexture(c), sprite = new THREE.Sprite(new THREE.SpriteMaterial({map:texture,depthTest:false})); sprite.scale.set(width,width/4,1); return sprite;
}
...
function rebuild() {
  ...
  for(const o of doc.objects) {
    ...
    const label = createLabel(text,Math.max(1.8,Math.min(o.width*1.7,3.5))); label.position.y = o.height+.55; g.add(label);
  }
}
function init3D() {
  try {
    ...
    renderer.setAnimationLoop(()=>{if(mode!=='print'){controls.update();renderer.render(scene,camera);}});
...
function fit(top=false) {
  ...
  const distance=Math.min(95,size*1.65/Math.min(camera.aspect,1)); controls.target.set(x,0,z);camera.position.set(x+(top?0:distance*.65),distance,z+(top?.01:distance*.85));controls.update();
}
function objectList() {
  const filter=$('filter').value;
  const list=game.objects.filter(o=>filter==='all'||o.type===filter);
  $('object-count').textContent=game.objects.length;
  $('object-list').innerHTML=list.length?list.map(o=>`<button class="object-row ${o.id===selected?'active':''} ${o.deckId?'card-row':''}" data-select="${esc(o.id)}"><i style="background:${o.color}"></i><span>${esc(o.name)}</span><small>${o.type}</small></button>`).join(''):'<p class="empty-list">No components here yet.<br>Add something to make it yours.</p>';
...
function inspector() {
  ...
  el.querySelectorAll('[data-field]').forEach(input=>{input.onchange=()=>{const key=input.dataset.field;let value=input.type==='number'?Number(input.value):input.value;if(key==='deckId')value=value||null;edit(key,value);};if(mode!=='build'&&!(mode==='play'&&['x','z'].includes(input.dataset.field)))input.disabled=true;});
...
function boardSvg() {
  ...
  return `<svg viewBox="0 0 800 650" role="img" aria-label="Printable board overview">${spaces.map(o=>{const x=400+(o.x-(minX+maxX)/2)*scale,y=325+(o.z-(minZ+maxZ)/2)*scale,w=o.width*scale,d=o.depth*scale;
    ...
    return `<g transform="translate(${x},${y}) rotate(${-o.rotation})" fill="${o.color}" stroke="#263b28" stroke-width="1">${shape}<text text-anchor="middle" y="4" fill="#142118" stroke="none" font-size="${Math.max(5,Math.min(13,w/Math.max(5,o.name.length)*1.3))}" font-family="sans-serif">${esc(o.name)}</text></g>`;}).join('')}</svg>`;
}
function printView() {
  ...
  for(let i=0;i<pieces.length;i+=12)pages+=`<article class="kit-page"><h1>Player pieces & counters</h1><p>1-inch cutouts · cut on dotted lines</p><div class="cutouts">${pieces.slice(i,i+12).map(o=>`<div class="piece-cut" style="background:${o.color}">${esc(o.name)}</div>`).join('')}</div></article>`;
  for(let i=0;i<cards.length;i+=6)pages+=`<article class="kit-page"><h1>Cards</h1><div class="print-cards">${cards.slice(i,i+6).map(o=>`<div class="print-card" style="background:${o.color}"><small>${esc(game.objects.find(d=>d.id===o.deckId)?.name||'Standalone card')}</small><h2>${esc(o.name)}</h2><p>${esc(o.text)}</p>${o.properties.map(p=>`<p><strong>${esc(p.key)}</strong> ${esc(p.value)}</p>`).join('')}</div>`).join('')}</div></article>`;
