...
function simplifyInspector() {
  ...
  if (placeholder) {
    ...
    intro.innerHTML=inspector.dataset.hasGame==='true'?'<span class="eyebrow">YOUR WORKSPACE</span><h2>Refine your game.</h2><p>Select any component to inspect its shape, color, and editable parts. Or describe a change in the conversation.</p><ol class="next-steps"><li>Inspect the board and rules</li><li>Try a guided match in Play</li><li>Refine, save, or export</li></ol><button class="wide" id="inspect-rules">Read game rules</button>':'<span class="eyebrow">MAKE IT YOURS</span><h2>Start with words.</h2><p>Describe what you imagine. Astra helps you turn it into a board, pieces, and rules.</p><ol class="next-steps"><li>Choose a game idea</li><li>Review and apply the draft</li><li>Try it, then refine it</li></ol><p class="muted">Already have a board? Click a piece to change it, or ask Astra to improve the whole game.</p><button class="wide" id="inspect-rules">Read game rules</button>';
  } else {
