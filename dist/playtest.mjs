...
export function evaluateGame(raw,{seeds=4}={}){
 ...
 if(c.spaceIds.length>16)return {supported:false,summary:'Automatic strategy diagnostics are limited to 16 grid spaces. Guided play remains available.'};
 const games=[];
 for(const policies of [['strategy','strategy'],['strategy','random'],['random','strategy']])for(let seed=1;seed<=Math.min(8,Math.max(1,seeds));seed++){
  const random=seededRandom(seed+800);let s=newMatch(c,seed),scoreless=0;
  while(!s.finished){const p=activePlayer(c,s),actions=legalActions(c,s),a=policies[p]==='strategy'?chooseAction(c,s):actions[Math.floor(random()*actions.length)],before=s.scores[0]+s.scores[1];s=applyAction(c,s,a);if(s.scores[0]+s.scores[1]===before)scoreless++;}
  games.push({seed,policies,winner:s.winner,scores:s.scores,turns:s.turn,scoreless});
 }
 ...
 return {supported:true,game:game.name,matches:games.length,wins,ties,meanTurns:Number((turns/games.length).toFixed(1)),scorelessPercent:Math.round(scoreless/turns*100),findings,games,limitations:'Small deterministic sample using a local two-ply strategy and seeded random actions. Routing has no randomness, so repeated strategy matches are identical. These results do not establish balance or fun.'};
}
